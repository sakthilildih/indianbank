...
def visualize_multiple_mules(df, pred_df, num_mules=3):
    ...
    for idx, (_, row) in enumerate(top_mules.iterrows(), 1):
        ...
        if fig:
            filename = f'outputs/visualizations/mule_accounts/mule_account_{idx}_{str(mule_account)[-6:]}.png'
...
def visualize_complete_mule_network(df, pred_df):
    ...
    plt.tight_layout()
    plt.savefig('outputs/visualizations/mule_accounts/mule_network_complete.png', dpi=300, bbox_inches='tight')
    print(f"\n✓ Saved: outputs/visualizations/mule_accounts/mule_network_complete.png")
    plt.close(fig)
...
def create_mule_comparison_chart(df, pred_df):
    ...
    plt.tight_layout()
    plt.savefig('outputs/visualizations/mule_accounts/mule_accounts_analysis.png', dpi=300, bbox_inches='tight')
    print(f"✓ Saved: outputs/visualizations/mule_accounts/mule_accounts_analysis.png")
    plt.close(fig)
...
╚════════════════════════════════════════════════════════════════════════════╝
    ...
    # Load data
    print("\n📥 Loading data...")
    test_txns = pd.read_csv('data/raw/test_transactions.csv')
    pred_df = pd.read_csv('outputs/visualizations/fraud_predictions.csv')
    ...
    print("="*80)
    print("\n📊 Generated Files:")
    print("   ✓ outputs/visualizations/mule_accounts/mule_account_1_*.png (Individual mule #1)")
    print("   ✓ outputs/visualizations/mule_accounts/mule_account_2_*.png (Individual mule #2)")
    print("   ✓ outputs/visualizations/mule_accounts/mule_account_3_*.png (Individual mule #3)")
    print("   ✓ outputs/visualizations/mule_accounts/mule_network_complete.png (Complete network)")
    print("   ✓ outputs/visualizations/mule_accounts/mule_accounts_analysis.png (Comparison dashboard)")
