    ...
    if epoch % 5 == 0:
        ...
        with torch.no_grad():
            ...
            if val_loss < best_loss:
                best_loss = val_loss
                counter = 0
                torch.save(model.state_dict(), "models/best_fraud_gnn_model.pth")
...
model.load_state_dict(torch.load("models/best_fraud_gnn_model.pth"))
model.eval()
with torch.no_grad():
    out = model(data.x, data.edge_index)
...
torch.save(model.state_dict(), "models/fraud_gnn_model.pth")
print("\nFinal Model saved to models/fraud_gnn_model.pth")
